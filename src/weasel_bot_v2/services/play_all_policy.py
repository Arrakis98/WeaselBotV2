...
class PlayAllPolicyService:
    ...
    def filter_tracks_for_play_all(
        ...
        if not exclusions:
            return PlayAllEligiblePool(total_indexed_mp3=len(tracks), eligible_tracks=tuple(tracks))

        policy = self.policy.get_policy(guild_id)
        exception_ids = set()
        if not policy.strict_exclusions:
            exception_ids = {
        ...
        for track in tracks:
            ...
            if track.id is not None and track.id in exception_ids:
                eligible.append(track)
        return PlayAllEligiblePool(total_indexed_mp3=len(tracks), eligible_tracks=tuple(eligible))
